...
To confirm count after loading:
    ...
    PY
Or:
    python -c "from data import db_connect; c = db_connect.connect_db(); print(list(c[db_connect.GEO_DB]['cities'].find().limit(2)))"
"""
import argparse
import json
import sys
...
def main():
    ...
    p.add_argument('--file', required=True, help='Path to JSON file (array of objects)')
    p.add_argument('--collection', required=True, help='Mongo collection name')
    p.add_argument('--db', default=db_connect.GEO_DB, help='Mongo DB name (optional)')
    p.add_argument('--drop', action='store_true', help='Drop collection before inserting')
    args = p.parse_args()
    ...
    if not isinstance(data, list):
        print('Seed file must contain a JSON array of objects', file=sys.stderr)
        sys.exit(2)
    ...
    if args.drop:
        print(f'Dropping collection {args.db}.{args.collection} (per --drop)')
        coll.drop()
    ...
    try:
        ...
        print(f'Inserted {len(res.inserted_ids)} documents into {args.db}.{args.collection}')
    except Exception as e:
